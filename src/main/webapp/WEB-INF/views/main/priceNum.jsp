...
<body>
				...
				<div class="site-navigation text-center dark">
					...
					<div class="menu-icons">
                     ...
                     <c:if test="${empty sessionScope.login}">
                        <a href="<%=pjName%>/login.do" class="user-profile"> <svg width="1em"
                     ...
                     <c:if test="${not empty sessionScope.login}">
                        <a href="<%=pjName%>/cart.do?cater=mypage&m_Id=${sessionScope.login}" class="user-profile"> <svg width="1em"
								height="1em" viewBox="0 0 16 16" class="bi bi-person"
								fill="currentColor" xmlns="http://www.w3.org/2000/svg">
								<path fill-rule="evenodd" d="M13 14s1 0 1-1-1-4-6-4-6 3-6 4 1 1 1 1h10zm-9.995-.944v-.002.002zM3.022 13h9.956a.274.274 0 0 0 .014-.002l.008-.002c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664a1.05 1.05 0 0 0 .022.004zm9.974.056v-.002.002zM8 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4zm3-2a3 3 0 1 1-6 0 3 3 0 0 1 6 0z" />
							</svg>
						...
						<!--  장바구니 버튼 -->
						<a href="<%=pjName%>/cart.do?m_Id=${sessionScope.login}" class="cart" > <span class="item-in-cart">2</span>
							<svg width="1em" height="1em" viewBox="0 0 16 16"
  ...
  <div class="untree_co-section pt-3">
                ...
                <ul class="list-unstyled">
                  <li>
                   <c:if test="${pageMaker.prev}">
                    <a href="<%=pjName%>/main/priceNum.do${pageMaker.makeQuery(pageMaker.startPage - 1)}&sc_Id=${param.sc_Id}&ct_Id=${param.ct_Id}">
    ...
    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
    	<li><a href="<%=pjName%>/main/priceNum.do${pageMaker.makeQuery(idx)}&sc_Id=${param.sc_Id}&ct_Id=${param.ct_Id}">${idx}</a></li>
    </c:forEach>
                  <li>
                     <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
                    <a href="<%=pjName%>/main/priceNum.do${pageMaker.makeQuery(pageMaker.endPage + 1)}&sc_Id=${param.sc_Id}&ct_Id=${param.ct_Id}">
